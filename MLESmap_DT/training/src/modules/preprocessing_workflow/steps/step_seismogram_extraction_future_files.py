...
def step_seismogram_extraction_future_object(path_to_folders, seismogram_files, output_folder=None, save_intermediate_files=False):
    f_files_out = []
    if output_folder is not None:
        if not os.path.exists(output_folder):
            os.mkdir(output_folder)
    for f_Folder in seismogram_files:
        f_files_out.append([])
        try:
            f_Stations = []
            for station in f_Folder:
                f_files_out[-1].append([])
                folder_Out = output_folder+"/step_1"
                if not os.path.exists(folder_Out):
                    os.mkdir(folder_Out)
                for seismogram_file in station:
                    j = seismogram_file.split("/")[-1]
                    f1 = j.split("_")
                    src = f1[4]
                    stat = f1[2]
                    f3 = f1[5].split(".")
                    ...
                    seismogram_station_extraction_only_one_file(seismogram_file, compss_file,
                                                                write_pandas_df=save_intermediate_files)
