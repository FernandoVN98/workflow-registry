...
def step_srf_extraction_future_object(path_to_folders, srf_files, output_folder=None, save_intermediate_files=False):
    for srf_file in srf_files:
        pass
    f_Folders = []
    f_files_out = []
    for f_Folder in srf_files:
        f_files_out.append([])
        for files_in_station in f_Folder:  # Loop that goes through the stations
            srf_in_folder = []
            stations = []
            stations_compss = []
            ...
            if not os.path.exists(folder_Out):
                os.mkdir(folder_Out)
            f_stat = files_in_station[0].split("/")[-3].split('_')
            stat = f_stat[1]
            name_Out = folder_Out + '/srf_extraction_' + stat + '_src.csv'
            name_compss = 'file_srf_extraction_' + stat + '_src.csv'
            srf_out = compute_srf_file(files_in_station, folder_Out, name_Out)
...
def compute_srf_file(srf_files,folder_Out,name_Out):
    if srf_files != []:
        srf_out_station = srf_station_extraction_files(srf_files)
    else:
        print('station-no-post-pro',folder_Out)
        srf_out_station = pd.DataFrame()
    return srf_out_station

def srf_station_extraction_files(f_SRF):
    srf_out_station = []
