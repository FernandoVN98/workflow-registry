...
def srf_extraction(srf_file2):
    ...
    return data_final
def step_1(f_Seismograms, path_to_folders, output_folder=None, save_intermediate_files=False):
    f_Folders = []
    f_files_out = [[]]
    ...
    for (dirpath, dirnames, filenames) in walk(path_to_folders):
        f_Folders.extend(dirnames)
        break
    for seismograms_station in f_Seismograms:
        f_files_out[-1].append([])
        for seismogram_file in seismograms_station:
            if output_folder is not None:
...
def compute_srf_nuevo(files_SRF,folder_Out,name_Out):
    f_SRF = []
    if files_SRF is not None:
        if len(files_SRF) > 0:
            f_SRF = files_SRF
    print("F SRF")
    print(f_SRF)
...
def step_2_nuevo(path_to_folders, seismogram_files, output_folder=None, save_intermediate_files=False):
    f_files_out = [[]]
    for seismogram_station in seismogram_files:
        stations_ = []
        stations_compss = []
        srf_in_folder = []
        if len(seismogram_station) > 0:
            stations = seismogram_station[0]
        else:
            continue
        f_stat = stations.split("/")[-3]
        stat = f_stat.split("_")[1]
        path_Stat = "/".join(stations.split("/")[:-2])#path_to_file + i + '/'
        if output_folder is not None:
        ...
        srf_out = None
        if flag == True:
            srf_out = compute_srf_nuevo(seismogram_station, folder_Out, name_Out)
        srf_in_folder.append(srf_out)
        stations_.append(name_Out)
        stations_compss.append(name_Out)
        for out_station, name_station, name_compss in zip(srf_in_folder, stations_, stations_compss):
...
def step_3_nuevo(first_step_files, second_step_files, path_fo_file, CyberS_sites_temp, output_folder=None, save_intermediate_files=False):
    ...
    for file_folder_output_PSA, file_folder_output_srf in zip(first_step_files, second_step_files):
        print("PRIMER BUCLE")
        print(file_folder_output_PSA)
        print(file_folder_output_srf)
        for file_station_psa, file_station_output in zip(file_folder_output_PSA, file_folder_output_srf):
            print("SEGUNDO BUCLE")
            print(file_station_psa)
            print(file_station_output)
            ...
            #    "./inputFiles/CyberShake_Sites.csv"
            #)
            print("ANTES DE SS PSA COMPSS NUEVO")
            print(file_station_psa)
            print(files_station_output)
...
def merged_steps_preprocess_mlesmap(path_to_folders, seismogram_files, grm_files, output_folder, table_CS_Ruptures, save_intermediate_files=False):
    CyberS_sites_temp = table_CS_Ruptures
    files_out_first_step = step_1(grm_files, path_to_folders, output_folder=output_folder, save_intermediate_files=save_intermediate_files)
    files_output_second_step = step_2_nuevo(path_to_folders, seismogram_files, output_folder, save_intermediate_files=save_intermediate_files)
    print("FILES OUT FIRST STEP")
    print(files_out_first_step)
    print("FILE SOUTPUT SECOND STEP")
    print(files_output_second_step)
    ...
    print("ONE SECOND PSAS")
    print(one_second_PSAs)
