from modules.cybershake_workflows.utils import SQLiteHandler, Ruptures, Ruptures_FM
...
@task(crunPath=FILE_OUT)
def generateCyberShakeCFG_compss(config, crunPath):
    # Generate the CS CFG file
    with open(crunPath, 'w') as f:
    ...
    try:
        ...
        if not 'ruptures' in config["input"]["ERF"].keys():
            ...
            if "focalMechanism" in config["compute"]["setup"].keys(): 
                ...
                rupt = Ruptures_FM(config["input"]["ERF"]["ruptures"] + "Ruptures_erf"
        ...
        dep2, awp_reformat_file_y, header_file_y = step_postAWP(config["input"]["cyberShake"]["path"], config["output"]["cRunpath"], "y", siteSN,
                config["input"]["model"]["box"], config["input"]["model"]["gridOut"], runID, config["compute"]["setup"]["frequency"],
                config["compute"]["setup"]["sourceFrequency"], config["input"]["cyberShake"]["gravesPitarka"], config["input"]["ERF"]["id"],
        ...
        output_dir = step_runDS(config["input"]["cyberShake"]["path"], config["input"]["database"]["path"], file_cfg, rupture_file_list, 
...
def cybershake_workflow(configuration_file, **kwargs):
    try:
        ...
        for runID, site in zip(runIDs, config["input"]["sites"]):
            output_dir = simulation_site(copy.deepcopy(config), runID, site)
