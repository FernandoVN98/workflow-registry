...
class SQLiteHandler():
 ...
 r.Source_ID=rv.Source_ID and r.Rupture_ID = rv.Rupture_ID group by r.Source_ID, r.Rupture_ID;'
    ...
    def importData(self, path):
        ...
        print("###" + " Importing data from CSV files on '" + path + "'" )
        ...
        for file in os.listdir(path):
            ...
            i = 1
            batch = []
            fields_num = len(list(lines[0].split(",")))
            fields = "(" + ((fields_num-1)*"?,") + "?)"
            query = queryHeader + queryWhere + queryWhat + " values " + fields
            rows = len(lines[1:])
            for values in lines[1:]:

                # Build query
                batch.append(make_tuple("'" + values.replace(",", "','") + "'"))
